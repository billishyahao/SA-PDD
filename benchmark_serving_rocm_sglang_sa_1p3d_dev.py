...
ssh -i /home/amd/.ssh/id_rsa {serverd3_ip}                                    \
docker run --rm -d --network {network_name} --ipc host --name {serverd3_name} \
    --privileged --cap-add=CAP_SYS_ADMIN --device=/dev/kfd --device=/dev/dri --device=/dev/mem \
    --group-add render --cap-add=SYS_PTRACE --security-opt seccomp=unconfined \
    -e HUGGINGFACE_HUB_CACHE=/models -e MODELSCOPE_CACHE=/models \
    -v /home/amd/models:/models -v /home/amd/billhe:/billhe --workdir /billhe \
...
start_time=$(date +%s)
while true; do
    if curl -s "{serverd3_ip}:{d3port}/v1/models" > /dev/null; then
      echo "Server {serverd3_ip} on port {d3port} is ready."
      break
    fi
    now=$(date +%s)
    if [ $((now - start_time)) -ge $timeout_seconds ]; then
      echo "Timeout waiting for server {serverd3_ip} on port {d3port}"
      exit 1
    fi
    sleep 1
done
...
docker run --rm -d --network {network_name} --ipc host --name {serverlb_name} \
    ...
    -v /home/amd/models:/models -v /home/amd/billhe:/billhe --workdir /billhe \
    {image_name} \
    python -m sglang.srt.disaggregation.mini_lb --prefill http://{serverp_ip}:{pport} --decode http://{serverd1_ip}:{d1port} http://{serverd2_ip}:{d2port} http://{serverd3_ip}:{d3port} --host {serverlb_ip} --port {lbport}
...
ssh -i /home/amd/.ssh/id_rsa {serverd3_ip} docker stop {serverd3_name}
