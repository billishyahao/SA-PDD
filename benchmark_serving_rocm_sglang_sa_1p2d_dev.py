...
docker run --rm -d --network {network_name} --ipc host --name {serverlb_name} \
    ...
    -v /home/amd/models:/models -v /home/amd/billhe:/billhe --workdir /billhe \
    {image_name} \
    python -m sglang.srt.disaggregation.mini_lb --prefill http://{serverp_ip}:{pport} --decode http://{serverd1_ip}:{d1port} http://{serverd2_ip}:{d2port} --host {serverlb_ip} --port {lbport}
