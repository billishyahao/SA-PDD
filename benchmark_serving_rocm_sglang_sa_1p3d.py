...
docker run --rm -d --network {network_name} --ipc host --name {serverd_name}3 \
    --privileged --cap-add=CAP_SYS_ADMIN --device=/dev/kfd --device=/dev/dri --device=/dev/mem \
    --group-add render --cap-add=SYS_PTRACE --security-opt seccomp=unconfined \
    -e HUGGINGFACE_HUB_CACHE=/models -e MODELSCOPE_CACHE=/models \
    -v /home/amd/models:/models -v /home/amd/billhe:/billhe --workdir /billhe \
...
printf "RESULT_FILENAME=%s\n" "{result_filename}"
while ! docker logs {serverd_name}3 2>&1 | grep -q "The server is fired up and ready to roll"; do
    sleep 1
    if docker logs {serverd_name}3 2>&1 | grep -q "ERROR"; then
        docker logs {serverd_name}3 >& "failed_runs/{result_filename}.log"
        docker stop {serverd_name}3;docker network rm {network_name}
        exit 1
    fi
done
...
docker run --rm -d --network {network_name} --ipc host --name {serverlb_name} \
    ...
    -v /home/amd/models:/models -v /home/amd/billhe:/billhe --workdir /billhe \
    {image_name} \
    python -m sglang.srt.disaggregation.mini_lb --prefill http://{serverp_name}:{port} --decode http://{serverd_name}1:{port} http://{serverd_name}2:{port} http://{serverd_name}3:{port}  --host 0.0.0.0 --port {port}
...
docker stop {serverp_name};docker stop {serverd_name}1;docker stop {serverd_name}2;docker stop {serverd_name}3;docker stop {serverlb_name};docker network rm {network_name}
sleep 5
'''
